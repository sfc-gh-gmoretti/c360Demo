...
export async function POST(request: NextRequest) {
  ...
  const snowflakeConfig: SnowflakeConfig = {
    ...
    authMethod: requestConfig.authMethod || savedConfig?.snowflake?.authMethod || "password",
    password: requestConfig.password || savedConfig?.snowflake?.password,
    pat: requestConfig.pat || savedConfig?.snowflake?.pat,
    privateKey: savedConfig?.snowflake?.privateKey,
  };
